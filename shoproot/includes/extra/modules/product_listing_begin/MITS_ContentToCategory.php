...
/**
 * --------------------------------------------------------------
 * File: MITS_ContentToCategory.php
 * Date: 05.12.2024
 * Time: 20:17
 *
 * Author: Hetfield
        ...
        foreach ($mits_contentsites as $mits_prod_content) {
            ...
            if (is_array($mits_content_data) && sizeof($mits_content_data) > 0) {
                ...
                if ($mits_content_counter == 1) {
                    $module_smarty->assign('MITS_CONTENT_ID', $mits_prod_content);
                    $module_smarty->assign('MITS_CONTENT_HEADING', $mits_content_heading);
                    $module_smarty->assign('MITS_CONTENT_TEXT', $mits_content_data['content_text']);
                }
                $module_smarty->assign('MITS_CONTENT_ID_' . $mits_content_counter, $mits_prod_content);
                $module_smarty->assign('MITS_CONTENT_HEADING_' . $mits_content_counter, $mits_content_heading);
                $module_smarty->assign('MITS_CONTENT_TEXT_' . $mits_content_counter, $mits_content_data['content_text']);
                ...
                $module_smarty->assign('mits_content_data', $mits_prod_content_data);
            }
