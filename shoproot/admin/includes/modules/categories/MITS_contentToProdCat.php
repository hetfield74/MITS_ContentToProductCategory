...
class MITS_contentToProdCat
{
    ...
    public function install(): void
    {
        ...
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('" . $this->name . "_ONLY_ACTIVE', 'false','6', '1','xtc_cfg_select_option(array(\'true\', \'false\'), ', now())");
    ...
    public function remove(): void
    {
        ...
        $prod_modul_code = 'MITS_ContentforProductsListing';
        if (defined('MODULE_PRODUCT_' . strtoupper($prod_modul_code) . '_STATUS')) {
            $o_installed_array = explode(';', MODULE_PRODUCT_INSTALLED);
            $installed_array = array();
            foreach ($o_installed_array as $value) {
                if ($value != $prod_modul_code . '.php') {
                    $installed_array[] = $value;
                }
            }
            xtc_db_perform(TABLE_CONFIGURATION, array('configuration_value' => implode(';', $installed_array)), 'update', "`configuration_key` = 'MODULE_PRODUCT_INSTALLED'");
            xtc_redirect(xtc_href_link(FILENAME_MODULES, 'set=product&module=' . $prod_modul_code . '&action=removeconfirm'));
        }
    }
